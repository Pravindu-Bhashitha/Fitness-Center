...
import { createBooking, getBookingById, listBookings, createMessage, listMessages, listTrainers, getTrainerById, createTrainer, updateTrainer, deleteTrainer, listClasses, getClassById, createClass, updateClass, deleteClass, initDatabase, } from './db.js';
const app = express();
...
app.get('/api/bookings', wrap(async (_request, response) => {
    response.json({ bookings: await listBookings() });
}));
app.get('/api/bookings/:id', wrap(async (request, response) => {
...
app.post('/api/bookings', wrap(async (request, response) => {
    const { type, trainerOrClass, specialty, date, time, duration, fullName, email, phone, goals = '', notes = '', } = request.body ?? {};
...
app.get('/api/messages', wrap(async (_request, response) => {
    response.json({ messages: await listMessages() });
}));
app.post('/api/messages', wrap(async (request, response) => {
...
app.get('/api/trainers', wrap(async (_request, response) => {
    response.json({ trainers: await listTrainers() });
}));
app.get('/api/trainers/:id', wrap(async (request, response) => {
...
app.post('/api/trainers', wrap(async (request, response) => {
...
app.put('/api/trainers/:id', wrap(async (request, response) => {
    const trainerId = String(request.params.id);
    const { name, specialty, experience, certifications, bio, avatar } = request.body ?? {};
    if (!name || !specialty || !experience || !certifications || !bio || !avatar) {
        response.status(400).json({ message: 'Missing required trainer fields' });
...
app.delete('/api/trainers/:id', wrap(async (request, response) => {
    const trainerId = String(request.params.id);
    const trainer = await getTrainerById(trainerId);
    if (!trainer) {
        response.status(404).json({ message: 'Trainer not found' });
...
app.get('/api/classes', wrap(async (_request, response) => {
    response.json({ classes: await listClasses() });
}));
app.get('/api/classes/:id', wrap(async (request, response) => {
...
app.post('/api/classes', wrap(async (request, response) => {
...
app.put('/api/classes/:id', wrap(async (request, response) => {
    const classId = String(request.params.id);
    const { name, schedule, duration, capacity, level, description, icon } = request.body ?? {};
    if (!name || !schedule || !duration || !capacity || !level || !description || !icon) {
        response.status(400).json({ message: 'Missing required class fields' });
...
app.delete('/api/classes/:id', wrap(async (request, response) => {
    const classId = String(request.params.id);
    const classItem = await getClassById(classId);
    if (!classItem) {
        response.status(404).json({ message: 'Class not found' });
